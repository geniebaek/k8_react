...
export default function FcstList() {
  ...
  const handleSelect=()=>{
    ...
    const tm=tdata.filter(item=>item['category']===selRef.current.value)
                           .map(item=><tr className="h-10 bg-white border-b hover:bg-gray-50 cursor-pointer" key={item.category+item.fcstDate+item.fcstTime}>
                                          ...
                                          <td className="text-center">{item.fcstValue}{code.단위}</td>
                                          </tr>
  ...
  return (
    <div className="w-full flex flex-col justify-start items-center">
      ...
      <table className="w-10/12 text-sm text-left rtl:text-right text-gray-500 ">
        <thead className=" text-lg font-bold text-white  bg-black">
            <tr>
